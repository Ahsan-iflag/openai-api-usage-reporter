...
Slice 0-5 では、Frontend（React + Vite）プロジェクトを初期化します。
以下が設定されます：
- Vite + React 19 プロジェクト
- Material-UI v5 の設定
- TanStack Query（React Query）の設定
...
スキルが完了したら、以下をユーザーに説明してください：
1. Frontend プロジェクト構造（src/pages, src/features など）
2. MUI、TanStack Query、orval の役割
3. npm scripts（npm run dev, npm run build など）
4. 開発サーバーの起動確認方法（http://localhost:5173）

その後、チェックリストを確認させてください：
- [ ] Vite + React プロジェクト初期化完了
- [ ] Material-UI セットアップ完了
- [ ] TanStack Query 設定完了
- [ ] orval 設定確認
- [ ] npm run dev で開発サーバー起動確認（http://localhost:5173）

**質問と確認**:

「Frontend の初期設定で質問や変更したい部分はありますか？」
